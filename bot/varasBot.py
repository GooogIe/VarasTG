...
class Bot:
    ...
    def parseMsg(self):
		...
		if self.pluginHandler.isAPlugin(command):            					#If the first element of the list is in the list of pluginf
			"""if self.debug == "False" or self.debug == False:
				try:
					self.sendMessage(self.pluginHandler.runPlugin(cmd)) 		#Call callPlugin method and send the result
				except:                                     					#Except shit not to see errors
					utils.alert("Error while performing "+command+" ("+str(self.user_id)+")")
					self.sendMessage("Error while performing "+command)
			else:"""
